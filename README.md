# Program Exit Assessment
## Gautam Batta

[GBatta2005@gmail.com](mailto:gbatta2005@gmail.com)

Heritage College

Graduated Computer Science Student

The Program Exit Assessment is a portfolio to display what we have learned in this program.
This portfolio will be helpful when we apply for jobs, as we will have something to show 
our knowledge.

<hr>

# The Heritage Computer Science Program

## First Year
Programming I<br>
Web Programming I<br>
Hardware & Operating Systems<br>
Math for Computer Science<br>
---<br>
Programming II<br>
Web Programming II<br>
Networks<br>
Business In The Workplace<br>

## Second Year
Programming III<br>
Web Programming III<br>
Database Management I<br>
Systems Analysis<br>
---<br>
Web Programming IV<br>
Web Programming V<br>
Database Management II<br>

## Third Year
Advanced Topics in Computer Science I<br>
Web Programming VI<br>
Development Project I<br>
Systems Maintenance<br>
---<br>
Advanced Topics in Computer Science II<br>
IT Security<br>
Development Project II<br>

<hr>

![math](https://user-images.githubusercontent.com/119340575/205943872-9c3b14a7-07db-4aac-a579-4cba24985b35.svg)

<hr>

# Personal Projects:

<br>

## SidGautamJS: 
A fully-functioning JavaScript library. It works very well and can be useful, but it has a steep learning curve. 

[GitHub Repo](https://github.com/GautamBatta73/SidGautam/tree/main/Versions/JavaScript)

<br>

## SidGautamAI: 
A great ChatGPT alternative, using DeepSeek and Llama. It works very well and has a simple and pretty UI. 

[GitHub Repo](https://github.com/GautamBatta73/SidGautam/tree/main/Versions/AI_Chat_Assistant)<br>

Currently Deployed at [SidGautam.ca/ai-chat](https://sidgautam.ca/ai-chat/)

<br>

## SidGautamOS: 
A debian-based Linux distribution, that I made from about as scratch as you can get. 

[GitHub Repo](https://github.com/GautamBatta73/SidGautam/tree/main/Versions/Linux_Distro)<br>
