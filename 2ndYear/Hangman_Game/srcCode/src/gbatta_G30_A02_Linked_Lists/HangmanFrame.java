...
public class HangmanFrame extends JFrame {
	...
	public HangmanFrame() throws Exception {
		...
		saveExitItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (message.charAt(0) == '!')
						JOptionPane.showMessageDialog(contentPane, message.substring(1), "Invalid",
								JOptionPane.INFORMATION_MESSAGE);
					else
						JOptionPane.showMessageDialog(contentPane, "An Unexpected Error Occured: " + message, "Error",
		...
		exitItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					JOptionPane.showMessageDialog(contentPane, "An Unexpected Error Occured: " + e1.getMessage(),
		...
		scoreItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				scores.setPreferredSize(new Dimension(scoresTable.getPreferredSize().width + 150,
						scoresTable.getPreferredSize().height + 25));

				JOptionPane.showMessageDialog(contentPane, scores, "ScoreBoard", JOptionPane.INFORMATION_MESSAGE);
			} // actionPerformed(ActionEvent)
		});
		...
		hintItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					JOptionPane.showMessageDialog(contentPane, "Your Hint is '" + hangman.getHint() + "'", "Hint",
							JOptionPane.INFORMATION_MESSAGE);
					refresh();
				} // try
				catch (Exception e1) {
		...
		aboutItem.setForeground(new Color(0, 0, 0));
		aboutItem.setFont(new Font("Arial", Font.PLAIN, 16));
		...
		aboutItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				aboutText.setPreferredSize(new Dimension(600, 250));

				String rulesText = "<span style='font-size:16px;'><b>Rules</b></span><ol>"
						+ "<li>The game thinks of a word, and the player tries to guess it by suggesting letters.</li>"
						+ "<li>The word to guess is represented by a row of dashes, one for each letter.</li>"
						+ "<li>The player has 6 chances to guess the word by suggesting a letter that they think might be in the word.</li>"
						+ "<li>If the letter is present in the word, it is revealed in all its correct positions. If the letter is not present, a body part of a hanged figure is decomposed.</li>"
						+ "<li>The game moves to the next word when the player successfully guesses the word or when the full hangman figure is decomposed (i.e., after a certain number of incorrect guesses).</li></ol>"
						+ "<span style='font-size:14px;'><b>This game was made by <u>Gautam Batta</u></b></span>";

				aboutText.setText("<html>" + rulesText + "</html>");

				JPanel aboutPanel = new JPanel();
				aboutPanel.add(aboutText);

				JOptionPane.showMessageDialog(contentPane, aboutPanel, "About", JOptionPane.INFORMATION_MESSAGE);
			} // actionPerformed(ActionEvent)
		});
					...
					if (!fldGuess.getText().isBlank()) {
						if (!hangman.guess(fldGuess.getText().charAt(0)))
							JOptionPane.showMessageDialog(contentPane,
									"'" + fldGuess.getText().charAt(0) + "' is Incorrect!", "Invalid",
									JOptionPane.INFORMATION_MESSAGE);
					} else
		...
		btnLoad.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				boolean valid = false;
				String name = fldName.getText().toLowerCase().trim();
				try {
					hangman = HangmanSaveLoad.loadFromFile(name);
					valid = true;
				} // try
					...
					try {
						int choice = JOptionPane.showOptionDialog(contentPane,
								...
								"New Game/Save Not Found", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
								null, new String[] { "Yes", "No" }, "Option 1" // Default selection
						);

						if (choice == JOptionPane.YES_OPTION) {
							HangmanSaveLoad.saveToFile(hangman, name);
							valid = true;
						} // if (choice == JOptionPane.YES_OPTION)
					} // try
					catch (Exception e2) {
						...
						if (message.charAt(0) == '!')
							...
							JOptionPane.showMessageDialog(contentPane, "An Unexpected Error Occured: " + message,
									"Error", JOptionPane.ERROR_MESSAGE);
					} // catch (Exception)
			...
			public void run() {
				try {
					...
					if (message.charAt(0) == '!')
						...
						JOptionPane.showMessageDialog(null, "An Unexpected Error Occured: " + message, "Error",
								JOptionPane.ERROR_MESSAGE);
	...
	private void refresh() {
		...
		hangIMG = new ImageIcon(new ImageIcon("./hang" + hangman.getIncorrectSize() + ".png").getImage()
				.getScaledInstance(lblHangIMG.getWidth(), lblHangIMG.getHeight(), java.awt.Image.SCALE_SMOOTH));

		lblHangIMG.setIcon(hangIMG);

		if (hangman.getIncorrectSize() > 5) {
			JOptionPane.showMessageDialog(this, "The Man Was Hanged!\n The Word Was: " + hangman.getCurrentAnswer(),
					"You Lost!", JOptionPane.INFORMATION_MESSAGE);
		} // if (hangman.getIncorrectSize() > 5)
		else if (hangman.hasWon()) {
			player.setGamesWon(player.getGamesWon() + 1);
			JOptionPane.showMessageDialog(this, "You Got The Word!", "Congratulations!",
					JOptionPane.INFORMATION_MESSAGE);
		}
		...
		if (hangman.getIncorrectSize() > 5 || hangman.hasWon()) {
			try {
				...
				JOptionPane.showMessageDialog(this, "An Unexpected Error Occured: " + e.getMessage(), "Error",
						JOptionPane.ERROR_MESSAGE);
			} // catch (Exception)
