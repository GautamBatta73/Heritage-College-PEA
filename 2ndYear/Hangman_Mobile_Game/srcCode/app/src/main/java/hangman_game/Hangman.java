...
public class Hangman implements Serializable {
    private final SinglyLinkedList<String> allAnswers;
    ...
    public void setFileContent(String file) throws Exception {
        if (file != null && !file.isBlank()) fileContent = file;
        else throw new FileNotFoundException("The Word File had an unexpected error!");

        setAllAnswers();
    } //setFileName(String)

    private void setAllAnswers() throws Exception {
        Scanner scanFile = new Scanner(fileContent);
        scanFile.useDelimiter("\\r\\n");

        while (scanFile.hasNext()) {
            String temp = scanFile.nextLine();
            if (temp.length() < 22) allAnswers.add(temp);
        ...
        if (fileContent != null) {
            if (allAnswers.getLength() > 0) {
                ...
                for (int i = currAns.length() - 1; i >= 0; i--) {
                    ...
                    if (l.matches(nonGuessable)) currentGuessedAnswer.add(currAns.charAt(i));
                    else currentGuessedAnswer.add('_');

                    currentAnswer.add(currAns.charAt(i));
                } // for (int i = currAns.length() - 1; i >= 0; i--)
            } else throw new Exception("!You have Solved All the Words!.");
        } //if (fileName != null)
        else throw new NullPointerException("Hangman word file has not been declared.");
                ...
                if (Character.isLetter(guess) || getCurrentAnswer().contains(guess + "")) {
                    ...
                    if (incorrectSize > -1 && incorrectSize < 6) {
                        ...
                        for (int i = 0; i < incorrectGuesses.getLength(); i++) {
                            ...
                            if (curr == lowerGuess) found = true;
                        } //for (int i = 0; i < currentAnswer.getLength(); i++)
                    } //if (incorrectSize > -1 && incorrectSize < 7)
                    else throw new IndexOutOfBoundsException("!Out of hints and lives.");
    ...
    public String getIncorrectGuesses() {
        ...
        for (int i = incorrectGuesses.getLength() - 1; i >= 0; i--) {
            if (i > 0) strIncorrectGuesses += (incorrectGuesses.getElementAt(i) + ", ");
            else strIncorrectGuesses += incorrectGuesses.getElementAt(i);
        } //for (int i = 0; i < incorrectGuesses.getLength(); i++)
    ...
    public String getCurrentGuessedAnswer() {
        ...
        for (int i = 0; i < currentGuessedAnswer.getLength(); i++) {
            ...
            else strCurrentGuessedAnswer += currentGuessedAnswer.getElementAt(i);
        } //for (int i = 0; i < currentGuessedAnswer.getLength(); i++)
    ...
    public char getHint() throws Exception {
        ...
        if (!won) {
            if (fileContent != null) {
                ...
                if (incorrectSize > -1 && incorrectSize < 5) hint = getRandChar();
                else throw new IndexOutOfBoundsException("!Out of hints and lives.");
            } //if (fileName != null)
            else throw new NullPointerException("Hangman word file has not been declared.");
        } //if(!won)
        else throw new Exception("!You have won, so you can stop hanging the poor man.");

        if (hint == '0') throw new Exception("Could Not Get Hint.\n Please try again.");
    ...
    private void checkAnswer() {
        ...
        for (int i = 0; i < currentAnswer.getLength() && isEqual; i++) {
            String currAns = Normalizer.normalize(currentAnswer.getElementAt(i) + "", Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "");
            String currGuessAns = Normalizer.normalize(currentGuessedAnswer.getElementAt(i) + "", Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "");
            if (!currAns.equalsIgnoreCase(currGuessAns)) isEqual = false;
        } //for (int i = 0; i < currentAnswer.getLength() && isEqual; i++)
    ...
    private char getRandChar() {
        ...
        while ((randChar + "").matches(nonGuessable)) {
            ...
            if (!getCurrentGuessedAnswer().contains(rand + "") && rand != ' ') randChar = rand;
            else rand = ' ';
        } //while (rand == ' ')
